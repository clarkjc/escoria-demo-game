...
func _ready() -> void:
	...
	escoria.logger.connect("error_message_signal", self._on_error_message)


func _input(event: InputEvent):
	if event.is_pressed() and event is InputEventKey:
		if (event as InputEventKey).scancode == KEY_UP and not commands_history.is_empty():
		...
		if (event as InputEventKey).scancode == KEY_DOWN and not commands_history.is_empty():
			commands_history_current_id += 1
