...
func register_inventory_item(item: Node):
	item.connect(
		...
		self._on_mouse_left_click_inventory_item
	)
	item.connect(
		"mouse_double_left_inventory_item",
		self._on_mouse_double_left_click_inventory_item
	)
	item.connect(
		"mouse_right_inventory_item",
		self._on_mouse_right_click_inventory_item
	)

	item.connect(
		"inventory_item_focused",
		self._on_mouse_entered_inventory_item
	)
	item.connect(
		"inventory_item_unfocused",
		self._on_mouse_exited_inventory_item
	)
...
func register_background(background: ESCBackground):
	background.connect(
		"left_click_on_bg",
		self._on_left_click_on_bg
	)
	background.connect(
		"right_click_on_bg",
		escoria.inputs_manager._on_right_click_on_bg
	)
	background.connect(
		"double_left_click_on_bg",
		escoria.inputs_manager._on_double_left_click_on_bg
	)
	background.connect(
		"mouse_wheel_up",
		self._on_mousewheel_action.bind(1)
	)
	background.connect(
		"mouse_wheel_down",
		self._on_mousewheel_action.bind(-1)
