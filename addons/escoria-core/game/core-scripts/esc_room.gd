...
@export var camera_limits: Array \
	= [Rect2()]: set = set_camera_limits # (Array, Rect2)

# The editor debug display mode
@export var editor_debug_mode: EditorRoomDebugDisplay \
	= EditorRoomDebugDisplay.NONE: set = set_editor_debug_mode
...
func _draw():
	...
	for i in camera_limits.size():
		...
		draw_string(default_font, Vector2(camera_limits[i].position.x + 30,
			camera_limits[i].position.y + 30), str(i), 0, -1, 16,
